...
/**
 * A Realtime `error` event may be the documented, harmless response to a
 * `response.cancel` sent after the response already finished. Match it by the
 * client event_id rather than by timing alone so an unrelated server or session
 * error that happens near a barge-in is never swallowed.
 *
 * @param {string | undefined} errorEventId
 * @param {Map<string, number>} pendingCancelEvents
 * @param {number} now
 * @param {number} graceMs
 * @returns {boolean}
 */
export function isExpectedResponseCancelError(
  errorEventId,
  pendingCancelEvents,
  now,
  graceMs,
) {
  if (!errorEventId) return false;
  const sentAt = pendingCancelEvents.get(errorEventId);
