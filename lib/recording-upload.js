...
export const RECORDING_PART_REQUEST_TIMEOUT_MS = 180_000;
const MAX_ATTEMPTS = 5;
const RECORDING_REQUEST_TIMEOUT_MS = 25_000;
...
async function requestWithRetry(run, options = {}) {
  ...
  const timeoutMs = Math.max(10, Math.min(RECORDING_PART_REQUEST_TIMEOUT_MS,
    options.timeoutMs ?? RECORDING_REQUEST_TIMEOUT_MS));
  let lastError = null;
    ...
    try {
      ...
      const { response, payload } = await Promise.race([
        ...
        new Promise((_, reject) => {
          timer = setTimeout(() => {
            controller.abort();
            reject(new Error("RECORDING_UPLOAD_REQUEST_TIMEOUT"));
          }, timeoutMs);
        }),
      ]);
...
export function createLiveRecordingUploader(input) {
  ...
  const retryOptions = { attempts, sleep, timeoutMs: input.requestTimeoutMs };
  // A 4 MiB media part can legitimately take well over 25 seconds on a weak
  // mobile uplink. Keep short timeouts for control requests, but give byte
  // transfers enough time to finish before attempting the same immutable part.
  const partRetryOptions = {
    attempts,
    sleep,
    timeoutMs: input.partRequestTimeoutMs ?? RECORDING_PART_REQUEST_TIMEOUT_MS,
  };
  const chunks = [];
  const fullParts = [];
  ...
  let activeFinalizeCoverage = null;
  let failure = null;
  ...
  const ensureStarted = async () => {
    if (started) return;
    const response = await requestWithRetry((signal) => fetcher("/api/interviews/recording/upload/start", {
      method: "POST",
  ...
  const sendPart = async (part) => {
    ...
    const response = await requestWithRetry((signal) => fetcher("/api/interviews/recording/upload/part", {
      method: "PUT",
      headers: {
        ...commonHeaders,
  ...
  const runPumpSingleflight = () => {
    ...
    const tracked = pump()
      ...
      .finally(() => {
        if (activePump === tracked) activePump = null;
        ...
        if (fullParts.length > 0 && !failure && !completed && !finalizing) schedulePump();
      });
    activePump = tracked;
    // Background start/append callers intentionally do not await. Attach a
    // rejection observer immediately while preserving the rejecting promise
    // for retry/finalize callers.
    void tracked.catch(() => undefined);
    return tracked;
  };

  const schedulePump = () => {
    if (activePump || failure || completed || finalizing) return;
    void runPumpSingleflight();
  };
  ...
  return {
    ...
    async retry() {
      ...
      if (activeFinalize) return await activeFinalize;
      if (activePump) await activePump.catch(() => undefined);
      failure = null;
      await runPumpSingleflight();
      return { stored: completed, pendingParts: fullParts.length };
    },
    finalize(audioCoverage) {
      ...
      if (activeFinalize) {
        if (activeFinalizeCoverage !== audioCoverage) {
          throw new Error("録画の確定情報が一致しません。");
        }
        return activeFinalize;
      }
      activeFinalizeCoverage = audioCoverage;
      finalizing = true;
      const tracked = (async () => {
        if (activePump) await activePump.catch(() => undefined);
        ...
        sealedShape = shape;
        const sealResponse = await requestWithRetry((signal) => fetcher("/api/interviews/recording/upload/seal", {
          method: "POST",
          headers: { ...commonHeaders, "Content-Type": "application/json" },
        ...
        const completedResponse = await requestWithRetry((signal) => fetcher("/api/interviews/recording/upload/complete", {
          method: "POST",
        ...
        if (activeFinalize === tracked) {
          activeFinalize = null;
          activeFinalizeCoverage = null;
        }
      });
      activeFinalize = tracked;
      void tracked.catch(() => undefined);
      return tracked;
    },
    snapshot() {
...
export async function uploadRecordingResumably(input) {
  ...
  const started = await requestWithRetry((signal) => fetcher("/api/interviews/recording/upload/start", {
    method: "POST",
    headers: { ...commonHeaders, "Content-Type": "application/json" },
    body: JSON.stringify({
  ...
  for (let index = 0; index < totalParts; index += 1) {
    ...
    await requestWithRetry((signal) => fetcher("/api/interviews/recording/upload/part", {
      method: "PUT",
      headers: {
        ...commonHeaders,
        "Content-Type": "application/octet-stream",
  ...
  const completed = await requestWithRetry((signal) => fetcher("/api/interviews/recording/upload/complete", {
    method: "POST",
    headers: commonHeaders,
    signal,
  }), retryOptions);
