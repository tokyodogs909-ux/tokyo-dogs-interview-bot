...
export function reduceLocalMediaHealth(state, event) {
  if (["track_muted", "track_ended", "device_changed"].includes(event.type)) {
    return {
      blocked: true,
  ...
  // iOS and Android can briefly hide a page while showing browser chrome,
  // permission UI, or switching apps. Visibility alone does not prove that the
  // camera or microphone track ended. The page flushes MediaRecorder and
  // retries the same immutable upload when visible again; real mute/end/device
  // events above remain fail-closed.
  if (event.type === "page_hidden") return state;
