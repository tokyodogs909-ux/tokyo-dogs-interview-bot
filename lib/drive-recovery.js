...
export function recordingReplacementBlockCode(input) {
  if (input.oldReadErrorCode &&
    !["GOOGLE_DRIVE_API_404", "GOOGLE_DRIVE_API_410"].includes(input.oldReadErrorCode)) {
    return input.oldReadErrorCode;
  }
  if (input.oldRecording) {
    if (input.oldRecording.trashed === true) {
      return "GOOGLE_DRIVE_ARCHIVE_RECORDING_TRASHED_RESTORE_REQUIRED";
    }
    if (input.oldRecording.parents?.length !== 1 ||
      input.oldRecording.parents[0] !== input.expectedFolderId) {
      return "GOOGLE_DRIVE_ARCHIVE_RECORDING_MOVED_MANUAL_ATTENTION";
    }
    return "GOOGLE_DRIVE_ARCHIVE_FILE_READBACK_MISMATCH";
  }
  if (input.oldReadErrorCode) {
    if (!input.confirmedMissingAcrossDrive) {
      return "GOOGLE_DRIVE_RECORDING_REPAIR_CONFIRMATION_REQUIRED";
    }
    if (input.globalCandidates?.some((file) => file.trashed !== true)) {
      return "GOOGLE_DRIVE_ARCHIVE_RECORDING_MOVED_MANUAL_ATTENTION";
    }
    if (input.globalCandidates?.some((file) => file.trashed === true)) {
      return "GOOGLE_DRIVE_ARCHIVE_RECORDING_TRASHED_RESTORE_REQUIRED";
...
export function summarizeDriveArchives(interviews, recoverySessionIds = []) {
  ...
  // Recording continuity alerts may belong to an interrupted, not-yet-
  // completed interview. They still require immediate staff attention.
  const openAlerts = interviews.filter((item) => item.driveAlertStatus === "open").length;
  return {
